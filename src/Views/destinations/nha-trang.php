<?php
$pageTitle = 'Nha Trang - Thiên đường đảo và biển';

$pageStyles = [
  'https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.0/css/all.min.css',
  '/css/destination.css'
];

$pageScripts = [
  'https://cdn.jsdelivr.net/npm/swiper@11/swiper-bundle.min.js',
  '/js/destination.js'
];
?>

<section class="hero" style="background-image: url('/images/destinations/NhaTrang/review.jpg');">
  <div class="hero-overlay"></div>
  <div class="hero-text">
    <h1>Khám phá Nha Trang</h1>
    <p>Thành phố ven biển nổi tiếng với bãi cát trắng, làn nước trong xanh và hệ thống đảo tuyệt đẹp.</p>
    <a href="#highlights" class="btn-cta">Khám phá ngay</a>
  </div>
</section>

<main>
  <section class="content-section overview fade-in">
    <h2><i class="fas fa-umbrella-beach"></i> Tổng quan về Nha Trang</h2>
    <p>Nha Trang là thành phố biển nổi tiếng với những bãi biển xinh đẹp, vịnh biển trong xanh và các đảo hoang sơ. Nơi đây còn có các khu nghỉ dưỡng sang trọng, hải sản tươi ngon và nhiều hoạt động giải trí sôi động.</p>
  </section>

  <section class="tour-card fade-in">
    <div class="tour-image">
      <img src="/images/destinations/NhaTrang/vinh-nha-trang.jpg" alt="Vịnh Nha Trang">
      <div class="badge">Vịnh</div>
    </div>
    <div class="tour-details">
      <h2>Tour cano tham quan Vịnh Nha Trang</h2>
      <div class="rating">
        <span class="stars">★★★★★</span>
        <span>(342 đánh giá)</span>
      </div>
      <p>Tham quan các hòn đảo nổi tiếng như Hòn Mun, Hòn Tằm và lặn ngắm san hô dưới làn nước trong vắt.</p>
      <ul class="tour-info">
        <li><i class="fa-regular fa-clock"></i> 5 giờ</li>
        <li><i class="fa-solid fa-ship"></i> Du thuyền</li>
        <li><i class="fa-solid fa-water"></i> Biển & Đảo</li>
        <li><a href="#">Tour biển</a></li>
      </ul>
      <div class="price-and-button">
        <span class="price">Từ <strong>350.000đ/người</strong></span>
        <a href="#" class="btn-cta">Xem chi tiết</a>
      </div>
    </div>
  </section>

  <section class="tour-card fade-in">
    <div class="tour-details">
      <h2>VinWonders Nha Trang</h2>
      <div class="rating">
        <span class="stars">★★★★★</span>
        <span>(428 đánh giá)</span>
      </div>
      <p>Công viên giải trí lớn nhất Việt Nam với các trò chơi mạo hiểm, thủy cung, rạp chiếu phim 4D và công viên nước.</p>
      <ul class="tour-info">
        <li><i class="fa-regular fa-clock"></i> Cả ngày</li>
        <li><i class="fa-solid fa-ticket"></i> Vé trọn gói</li>
        <li><i class="fa-solid fa-child"></i> Phù hợp gia đình</li>
        <li><a href="#">Giải trí & Vui chơi</a></li>
      </ul>
      <div class="price-and-button">
        <span class="price">Từ <strong>880.000đ/người</strong></span>
        <a href="#" class="btn-cta">Xem chi tiết</a>
      </div>
    </div>
    <div class="tour-image">
      <img src="/images/destinations/NhaTrang/vinwonders.jpg" alt="VinWonders Nha Trang">
      <div class="badge">Giải trí</div>
    </div>
  </section>

  <section class="tour-card fade-in">
    <div class="tour-image">
      <img src="/images/destinations/NhaTrang/tam-bun.jpg" alt="Tắm bùn Nha Trang">
      <div class="badge">Thư giãn</div>
    </div>
    <div class="tour-details">
      <h2>Tắm bùn khoáng nóng Nha Trang</h2>
      <div class="rating">
        <span class="stars">★★★★☆</span>
        <span>(198 đánh giá)</span>
      </div>
      <p>Thư giãn trong bùn khoáng thiên nhiên giúp làm đẹp da, phục hồi thể chất và tinh thần.</p>
      <ul class="tour-info">
        <li><i class="fa-regular fa-clock"></i> 1.5 giờ</li>
        <li><i class="fa-solid fa-spa"></i> Chăm sóc sức khỏe</li>
        <li><i class="fa-solid fa-leaf"></i> Thiên nhiên</li>
        <li><a href="#">Spa & Thư giãn</a></li>
      </ul>
      <div class="price-and-button">
        <span class="price">Từ <strong>120.000đ/người</strong></span>
        <a href="#" class="btn-cta">Xem chi tiết</a>
      </div>
    </div>
  </section>

  <section class="content-section highlights" id="highlights">
    <h2>Điểm nổi bật</h2>
    <div class="cards">
      <div class="card">
        <img src="/images/destinations/NhaTrang/hai-san.jpg" alt="Hải sản Nha Trang">
        <h3>Hải sản tươi ngon</h3>
        <p>Nha Trang nổi tiếng với các món hải sản phong phú như tôm hùm, nhum, mực một nắng, cá mú hấp,...</p>
      </div>
      <div class="card">
        <img src="/images/destinations/NhaTrang/lan-bien.jpg" alt="Lặn biển ngắm san hô">
        <h3>Lặn biển ngắm san hô</h3>
        <p>Hòn Mun là một trong những địa điểm lặn biển ngắm san hô đẹp nhất Việt Nam với hệ sinh thái biển đa dạng.</p>
      </div>
      <div class="card">
        <img src="/images/destinations/NhaTrang/thap-ba.jpg" alt="Tháp Bà Ponagar">
        <h3>Tháp Bà Ponagar</h3>
        <p>Di tích Chăm cổ nằm trên đồi cao, nhìn ra vịnh Nha Trang tuyệt đẹp.</p>
      </div>
    </div>
  </section>

  <section class="gallery">
    <h2>Thư viện ảnh</h2>
    <div class="swiper-button-prev gallery-nav-prev"></div>
    <div class="swiper-button-next gallery-nav-next"></div>

    <div class="swiper gallery-swiper">
      <div class="swiper-wrapper">
        <?php
        $galleryImages = [
          ['vinh-nha-trang.jpg', 'Vịnh Nha Trang'],
          ['vinwonders.jpg', 'VinWonders'],
          ['tam-bun.jpg', 'Tắm bùn Nha Trang'],
          ['hai-san.jpg', 'Hải sản Nha Trang'],
          ['lan-bien.jpg', 'Lặn biển Nha Trang'],
          ['thap-ba.jpg', 'Tháp Bà Ponagar']
        ];

        foreach ($galleryImages as [$image, $alt]) {
          echo "<div class='swiper-slide'>";
          echo "<img src='/images/destinations/NhaTrang/{$image}' alt='{$alt}'>";
          echo "</div>";
        }
        ?>
      </div>
    </div>
  </section>
</main>
